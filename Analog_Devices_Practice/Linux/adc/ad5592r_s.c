...
struct ad5592rs_state {
	bool en;
	int chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7;
...
static int ad5592rs_read_channel(struct ad5592rs_state *st,
				 const struct iio_chan_spec *chan, int *val)
{
	u16 rx = 0, temp = 0;
	int ret;

	ret = ad5592rs_spi_write(st, AD5592R_S_CONF_ADC_SEQ_REG,
				 AD5592R_S_ADC_SEQ(chan->channel));
	if (ret) {
		dev_err(&st->spi->dev,
			"Error at writing on channel for ADC_SEQ");
		return ret;
	}
...
static irqreturn_t ad5592rs_trig_handler(int irq, void *p)
{
	...
	for_each_set_bit(bit, indio_dev->active_scan_mask,
		...
		if (ret) {
			dev_err(&st->spi->dev,
				"Error at reading channel in handler");
...
static int ad5592rs_probe(struct spi_device *spi)
{
	...
	ret = ad5592rs_spi_write(st, AD5592R_S_CONF_ADC_CONFIG_REG,
				 AD5592R_S_ADC_EN);
	if (ret) {
